...
namespace turtle {
    ...
    float tsin(float _Ang) {
        ...
        if (std::fabs(rad_ang) == 90.f) {
            return 1.0f * (rad_ang / 90.f);
        }
        else if (std::fabs(rad_ang) == 180.f) {
            return 0.0f;
        }
        else if (std::fabs(rad_ang) == 270.f) {
            return -1.0f * (rad_ang / 270.f);
        }
        else if (std::fabs(rad_ang) == 360.f) {
            return 0.0f;
        }
        else {
    ...
    Turtle::Turtle() : m_Ang{ 90.0l } {
        ...
        this->m_HalfWindowWidth = window_size.x / 2.0f;
        this->m_HalfWindowHeight = window_size.y / 2.0f;
    ...
    void Turtle::set_coord_scalar(float _Ang) {
        ...
        if (std::fabs(rad_ang) == 90.f) {
            ...
            this->m_Sin = std::sin(rad_ang);
            this->m_Cos = std::cos(rad_ang);
        }
    }
    ...
    void Turtle::movement_animation(const sf::Vector2f& _LastPos, const sf::Vector2f& _NewPos) {
        ...
        std::thread([&]() {
            for (int i = 0; i < (int)this->m_Speed; ++i) {
                ...
                sf::Vector2f new_pos = this->get_pos();

                m_ulMutex.lock();

                sf::Vector2f vec{ new_pos.x - pos.x, new_pos.y - pos.y };
                float vec_len = sqrt(vec.x * vec.x + vec.y * vec.y);

                sf::Vector2f vec_ox(0.0f, 1.0f);

                float alpha = acos((vec.x * vec_ox.x + vec.y * vec_ox.y) / (vec_len * 1.0f));

                sf::RectangleShape line(sf::Vector2f(vec_len, 1.0f));
                line.setFillColor(sf::Color::Black);
                line.setOrigin(sf::Vector2f(0.5f, 0.0f));
    ...
    void Turtle::set_pos(const float& x, const float& y) {
        if (this->m_TailDown) {
            ...
            float ang_degrees = (180.f / M_PI) * alpha;
